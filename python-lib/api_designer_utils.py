...
def get_params(config, client, project):
    ...
    assert confidence, "Confidence is empty"
    assert type(confidence) is float, "Confidence is not a float : %s " % type(
        min_threshold)
    assert (confidence >= 0) and (confidence <=
                                  1), "Confidence must be between 0 and 1"
    params["confidence"] = confidence

    env_name = 'plugin_object-detection-api_node'
...
def get_model_endpoint_settings(params):
    ...
    endpoint_settings["code"] = """import sys
import dataiku
...
from retinanet_model import find_objects_single

weights = os.path.join(model_folder_path, 'weights.h5')
labels_to_names = json.loads(open(os.path.join(model_folder_path, 'labels.json')).read())
...
def api_py_function(img_b64):
    ...
    img_by = BytesIO(img_b64_decode)
    img = Image.open(img_by)
    img_arr_rgb = cv2.cvtColor(np.array(img), cv2.COLOR_BGR2RGB)
    print("------ Info: Done loading image ------")

    print("------ Info: Start predicting ------")
    boxes, scores, labels = find_objects_single(model, img_arr_rgb, min_side=200, max_side=400)
    print("------ Info: Done predicting ------")
    
    print("------ Info: Start processing results ------")
    result = dict()
    k = 0
    for box, score, label in zip(boxes[0], scores[0], labels[0]):
        if score < %s:
            break
        result[k] = {"x1":str(box[0]), "y1":str(box[1]), "x2":str(box[2]), "y2":str(box[3]), "class_name":labels_to_names[label] , "confidence":str(score)}
        k = k + 1
    print("------ Info: Done processing results ------")

    return result""" % params.get("confidence")
